describe 'feature page', type: :feature, js: true do
  ...
  it 'shows the signature, kind and visibility of the feature' do
    ...
    expect(page).to have_css('.feature.is_stable')
  ...
  it 'documents the parameters of the feature' do
    expect(page).to have_css('.feature__param', text: 'target')
    expect(page).to have_css('.feature__param .tag', text: /optional/i)
  end

  it 'documents the return value of the feature' do
    expect(page).to have_css('#return-value')
    expect(page).to have_css('.feature__param.is_response', text: 'up.RenderJob')
  end

  it 'renders the prose of the feature, linking code references to their own page' do
    expect(page).to have_css('.feature__prose')
    expect(page).to have_css('.feature__prose a[href="/up.navigate"] code', text: 'up.navigate()')
  end
